...
export function freshRecord():RecordData{return {schemaVersion:1,id:uid(),vehicleId:VEHICLE.id,createdAt:now(),updatedAt:now(),started:false,checks:Object.fromEntries(CHECKS.map(c=>[c.id,{status:'not-inspected',notes:''}])),findings:[],photos:[],history:[],undo:[],rules:{minor:2,moderate:8,serious:25,critical:50}};}
