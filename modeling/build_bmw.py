"""Original BMW-330i-inspired inspection reference, built with Blender.
...
def mat(name, color, metallic=0, rough=.4, coat=0, emission=0):
    ...
    return m
PAINT=mat('Black metallic paint',(.012,.017,.023),.38,.25,.65)
...
CALIPER=mat('M blue brake caliper',(.02,.13,.40),.5,.32)
...
for name,p in pivots.items():
    ...
    obj['partId']=name;obj['model']='Original approximate 2021 BMW 330i M Sport reference';parts[name]=obj
...
for side,label in [(1,'left'),(-1,'right')]:
 for end,zc in [('front',-1.42),('rear',1.44)]:
    ...
    for i in range(10):
      a=i*2*pi/10
      for branch in [-1,1]:
        a0=a+branch*.035;a1=a+branch*.065
        ...
        dx=.015;perp=(cos(a1),-sin(a1));w0=.009;w1=.005
...
# Sedan-specific bodywork and details, authored from the supplied BMW references.
remove_prefixes=('Roof rail','GT roof spoiler','Spoiler dark','High-mounted brake','Rear fixed quarter','Quarter glass','D pillar','Rear wiper','Heated rear','Rear screen defroster','Upper grille','Hex grille','Oval reference','Oval blue','Front lower skid','Rear skid','Fog lens','Fog lamp','Rear lamp','Rear LED','Tail reverse','Infotainment','Display interface','Boxer engine','Central instrument','Gauge ')
...
# Reshape the rear roof and rear-door glass to the G20's three-box silhouette.
for obj in list(bpy.context.scene.objects):
    if obj.type in ('MESH','CURVE'):
        ...
        for v in obj.data.vertices:
            ...
            x,y,z=p.x,p.z,-p.y
            if y>1.16 and z>.15:
                z-=.50*min(1,(y-1.16)/.5)*min(1,(z-.15)/.85)
            v.co=inv@vec((x,y,z))
patch('Sloping sedan rear windscreen',lambda u,v:(lerp(-.85,.85,u)*(1-.09*v),lerp(1.22,1.73,v),lerp(1.54,.75,v)),30,18,GLASS,'rear-glass',.018)
for side in [-1,1]:
    tube('Sedan C pillar',[(side*.8,1.73,.73),(side*.87,1.47,1.15),(side*.94,1.18,1.59)],.048,PAINT,'roof')
patch('Rear deck trunk lid',lambda u,v:(lerp(-.92,.92,u),1.22+.017*sin(pi*u)-.03*v,lerp(1.54,2.11,v)),30,16,PAINT,'boot',.025)
tube('Trunk trailing lip',[(-.9,1.21,2.11),(0,1.23,2.12),(.9,1.21,2.11)],.018,PAINT,'boot')
for side in [-1,1]:
    kidney=[(side*(.055+.40*(.5+.5*cos(a))),1.01+.145*sin(a),-2.295) for a in [i*2*pi/64 for i in range(64)]]
    mesh('Kidney grille dark cavity',kidney,[tuple(range(64))],TRIM,'front-bumper',solid=.02)
    tube('Kidney grille chrome outline',kidney,.014,CHROME,'front-bumper',cyclic=True)
    for i in range(9):
        x=side*(.095+i*.037);tube('Vertical kidney grille slat',[(x,.903,-2.31),(x,1.115,-2.31)],.0065,DARKMETAL,'front-bumper')
    tube('L shaped rear lamp',[(side*.92,1.21,2.15),(side*.90,1.02,2.17),(side*.42,1.02,2.17)],.029,RED,'lights')
    box('Rear lamp smoked lens',(side*.68,1.10,2.13),(.5,.12,.065),GLASS,'lights',.025)
    tube('Rear light bar',[(side*.89,1.06,2.178),(side*.4,1.06,2.178)],.012,RED,'lights')
    tube('Blue door ambient light',[(side*.842,1.025,-.83),(side*.842,1.02,-.1)],.004,SCREEN,'cabin-trim','interior')
box('Wide iDrive display bezel',(.18,1.30,-.66),(.55,.235,.04),TRIM,'dashboard',.018,'interior')
box('Wide iDrive display',(.18,1.30,-.63),(.51,.20,.01),SCREEN,'dashboard',.008,'interior')
box('Digital instrument cluster',(-.48,1.22,-.60),(.46,.19,.015),SCREEN,'dashboard',.03,'interior')
tube('Blue dashboard ambient strip',[(-.76,1.05,-.625),(0,1.04,-.625),(.75,1.07,-.625)],.004,SCREEN,'dashboard','interior')
cyl('iDrive controller',(.09,.85,.27),.05,.025,CHROME,'console',axis=(0,1,0),surface='interior')
box('Longitudinal engine cover',(0,.88,-1.55),(.55,.20,.75),TRIM,'engine',.045)
# Round blue/white quartered emblems.
for z,y,part in [(-2.13,1.115,'bonnet'),(2.15,1.17,'boot')]:
    cyl('Roundel rim',(0,y,z),.052,.012,CHROME,part,axis=(0,0,1))
    cyl('Roundel black ring',(0,y,z+.009),.045,.012,TRIM,part,axis=(0,0,1))
    for q in range(4):
        points=[(0,y,z+.018)]+[(.032*cos(a),y+.032*sin(a),z+.018) for a in [q*pi/2+i*pi/32 for i in range(17)]]
        mesh('Quartered blue white emblem',points,[tuple(range(len(points)))],BADGE if q%2 else HEADLIGHT,part)
# Lower sedan stance and longer wheelbase, keeping circular wheel geometry.
for part,objects in members.items():
    for obj in objects:
        if obj.type=='CURVE':
            bpy.ops.object.select_all(action='DESELECT');obj.select_set(True);bpy.context.view_layer.objects.active=obj;bpy.ops.object.convert(target='MESH')
        ...
        for v in obj.data.vertices:
            ...
            if part.endswith('-wheel'):
                zc=-1.42 if 'front' in part else 1.44
                y=.36+(y-.43)*.87;z=zc*1.08+(z-zc)*.87
            else:y*=.84;z*=1.08
            v.co=inv@vec((x*.95,y,z))
for name,obj in parts.items():
    p=pivots[name];obj.location=vec((p[0]*.95,p[1]*.84,p[2]*1.08))
...
bpy.ops.object.select_all(action='SELECT')
bpy.ops.export_scene.gltf(filepath=str(OUT/'bmw-330i-inspection.glb'),export_format='GLB',use_selection=True,export_apply=True,export_extras=True,export_yup=True,export_animations=False,export_cameras=False,export_lights=False)
...
metadata={'name':'BMW 330i M Sport inspired inspection reference','license':'MIT; original geometry','exact_reconstruction':False,'authoring':'Blender '+bpy.app.version_string,'part_count':len(parts),'mesh_count':len(mesh_objects),'triangles':triangles,'parts':{name:{'pivot':list(pivots[name]),'meshes':len([o for o in mesh_objects if o.parent==parent])} for name,parent in parts.items()}}
(OUT/'bmw-model-metadata.json').write_text(json.dumps(metadata,indent=2))
...
scene=bpy.context.scene;scene.render.engine='CYCLES';scene.cycles.samples=16;scene.cycles.use_denoising=True;scene.render.resolution_x=1100;scene.render.resolution_y=800;scene.render.resolution_percentage=100
scene.view_settings.view_transform='AgX';scene.render.image_settings.file_format='PNG';scene.render.filepath=str(OUT/'bmw-330i-studio.png')
