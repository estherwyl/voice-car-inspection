...
export default function App(){
 ...
 if(page==='vehicles')return <div className="vehicle-landing"><header><span className="simple-brand">JARVICI</span></header><main><h1>Select a vehicle</h1><button className="vehicle-choice" onClick={()=>setPage('inspection')} aria-label="Select 2022 Subaru XV GT Edition"><img src="/models/subaru-xv-studio.png" alt="White Subaru XV reference model"/><span className="vehicle-choice-info"><span><small>2022 · GT Edition</small><strong>Subaru XV</strong></span><span className="vehicle-choice-action">{record.started?'Resume':'Inspect'}<ArrowRight size={18}/></span></span></button></main></div>;
 return <div className="simple-app">
  ...
  <section className={'voice-control '+voiceState} aria-label="Voice companion">
   ...
   <b className="voice-state-label">{label}</b>{voiceError&&<p className="simple-voice-error" role="alert">{voiceError}</p>}<small className="voice-engine">OpenAI voice · automatic annotation</small>
  </section>
  {panel&&<div className="simple-backdrop" onClick={()=>setPanel(null)}><section className={'simple-sheet '+(panel==='detail'?'details-sheet':'')} role="dialog" aria-modal="true" aria-label={panel==='report'?'Inspection report':panel==='detail'?'Component details':panel==='history'?'Edit history':'About voice inspection'} onClick={e=>e.stopPropagation()}>
   ...
   {panel==='about'&&<div className="simple-about"><p><strong>JARVICI</strong><br/>Just A Rather Very Intelligent Car Inspection</p><p>Tap the microphone. Name the part and what you see. The companion records it and updates the car.</p>
